...
src1 = 'extended-cohn-kanade-images/cohn-kanade-images'
dst1 = 'source_images'

src2 = 'Emotion_labels/Emotion'
dst2 = 'source_emotion'
...
for i in range(len(EMOTIONS)):
    subfolder = 'sorted_set/'+ EMOTIONS[i]
...
for i in range(len(index_to_remove)):
    file_name = f'dataset/neutral/{index_to_remove[i]}.jpg'
    print(file_name)
    try:
        os.remove(file_name)
    except OSError:
...
file_list = os.listdir('dataset/neutral')
file_num_list = [int(num.split('.')[0]) for num in file_list]
file_num_list.sort()
print(file_num_list)
 
for j in range(len(file_num_list)):
    current_file = 'dataset/neutral/'+str(file_num_list[j])+'.jpg'
    new_file = f'dataset/neutral/{j}.jpg'
...
subfolders = [name for name in os.listdir('dataset')
            if os.path.isdir(os.path.join('dataset', name))]

for folder_name in subfolders:
    sub_test_folder = os.path.join('dataset/testing/'+folder_name)
    if not os.path.exists(sub_test_folder):
        os.makedirs(sub_test_folder) 

    sub_train_folder = os.path.join('dataset/training/'+folder_name)
    if not os.path.exists(sub_train_folder):
        os.makedirs(sub_train_folder) 

    all_files = [f for f in listdir('dataset/'+folder_name) if isfile(join('dataset/'+folder_name, f))]

    training_files = all_files[:int(len(all_files)*SPLIT_RATE)]

    for item in training_files:
        move('dataset/'+folder_name +'/' + item, sub_train_folder+'/'+item)
    
    remain_files = [f for f in listdir('dataset/'+folder_name) if isfile(join('dataset/'+folder_name, f))]

    for item in remain_files:
        move('dataset/'+folder_name +'/' + item, sub_test_folder+'/'+item)
